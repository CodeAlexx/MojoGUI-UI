...
struct MouseEventInt(ImplicitlyCopyable, Movable):
    ...
    fn with_modifiers(mut self, shift: Bool, ctrl: Bool, alt: Bool, super_key: Bool) -> Self:
        """Set modifier keys."""
...
struct KeyEventInt(ImplicitlyCopyable, Movable):
    ...
    fn with_char(mut self, char_code: Int32) -> Self:
        """Set character code."""
    ...
    fn is_navigation_key(self) -> Bool:
        ...
        var k = self.key_code
        return (k == KEY_UP or k == KEY_DOWN or k == KEY_LEFT or k == KEY_RIGHT
                or k == KEY_HOME or k == KEY_END or k == KEY_PAGE_UP
                or k == KEY_PAGE_DOWN)

    fn is_editing_key(self) -> Bool:
        """Check if this is an editing key."""
        var k = self.key_code
        return (k == KEY_BACKSPACE or k == KEY_DELETE or k == KEY_INSERT
                or k == KEY_ENTER or k == KEY_TAB)
...
struct FocusEventInt(ImplicitlyCopyable, Movable):
    """Focus event data."""
    var gained: Bool  # True if gaining focus, False if losing
    var reason: Int32  # Reason for focus change (tab, click, etc.)
    
    fn __init__(out self, gained: Bool, reason: Int32 = 0):
        self.gained = gained
        self.reason = reason

struct ResizeEventInt(ImplicitlyCopyable, Movable):
    ...
    fn __init__(out self, old_width: Int32, old_height: Int32,
...
struct DragDropEventInt(ImplicitlyCopyable, Movable):
    ...
    fn __init__(out self, x: Int32, y: Int32, action: Int32):
        self.x = x
        self.y = y
        self.action = action
        self.data_type = ""
...
comptime DRAG_START = 0
comptime DRAG_MOVE = 1
comptime DRAG_DROP = 2
comptime DRAG_ENTER = 3
comptime DRAG_LEAVE = 4
comptime DRAG_CANCEL = 5

# Focus change reasons
comptime FOCUS_TAB = 0
comptime FOCUS_BACKTAB = 1
comptime FOCUS_CLICK = 2
comptime FOCUS_WHEEL = 3
comptime FOCUS_PROGRAMMATIC = 4
...
fn char_to_keycode(ch: String) -> Int32:
    ...
    var c = Int(ch.as_bytes()[0])

    # Letters (convert to uppercase key codes)
    if c >= ord("a") and c <= ord("z"):
        return Int32(c - ord("a") + ord("A"))
