...
# ColorPicker styles
comptime COLORPICKER_COMPACT: Int32 = 0
comptime COLORPICKER_WHEEL: Int32 = 1
comptime COLORPICKER_SQUARE: Int32 = 2
comptime COLORPICKER_SLIDERS: Int32 = 3

# Events
comptime EVENT_COLOR_CHANGED: Int32 = 1201
comptime EVENT_COLOR_PICKED: Int32 = 1202
comptime EVENT_COLOR_PREVIEW: Int32 = 1203

struct ColorPickerInt(Copyable, Movable):
    """
    ...
    """
    # Inlined base widget geometry/state (this widget uses x/y/width/height directly).
    var x: Int32
    var y: Int32
    var width: Int32
    var height: Int32
    ...
    fn is_point_inside(self, mouse_x: Int32, mouse_y: Int32) -> Bool:
        """Check if a point is inside the main widget bounds"""
        return (mouse_x >= self.x and mouse_x <= self.x + self.width and
                mouse_y >= self.y and mouse_y <= self.y + self.height)

    fn trigger_event(self, event_type: Int32, value: Int32):
        """Event hook (no-op placeholder preserved from original)."""
        pass

    fn set_color_rgb(mut self, r: Int32, g: Int32, b: Int32, a: Int32 = 255):
        """Set color using RGB values (0-255)"""
        self.color_r = max(Int32(0), min(Int32(255), r))
        self.color_g = max(Int32(0), min(Int32(255), g))
        self.color_b = max(Int32(0), min(Int32(255), b))
        self.color_a = max(Int32(0), min(Int32(255), a))
        self._rgb_to_hsv()
        self.trigger_event(EVENT_COLOR_CHANGED, self._pack_rgb())

    fn get_color_rgb(self) -> Tuple[Int32, Int32, Int32, Int32]:
        """Get current color as RGBA values"""
        return Tuple[Int32, Int32, Int32, Int32](self.color_r, self.color_g, self.color_b, self.color_a)

    fn set_color_hsv(mut self, h: Int32, s: Int32, v: Int32):
        """Set color using HSV values (H: 0-360, S: 0-100, V: 0-100)"""
        self.hue = max(Int32(0), min(Int32(360), h))
        self.saturation = max(Int32(0), min(Int32(100), s))
        self.brightness = max(Int32(0), min(Int32(100), v))
        self._hsv_to_rgb()
        self.trigger_event(EVENT_COLOR_CHANGED, self._pack_rgb())

    fn get_color_hsv(self) -> Tuple[Int32, Int32, Int32]:
        """Get current color as HSV values"""
        return Tuple[Int32, Int32, Int32](self.hue, self.saturation, self.brightness)

    fn set_color_hex(mut self, hex_value: Int32):
    ...
    fn set_alpha_mode(mut self, enabled: Bool):
    ...
    fn apply_preview_color(mut self):
    ...
    fn _unpack_rgb(self, packed: Int32) -> Tuple[Int32, Int32, Int32]:
        """Unpack RGB from single integer"""
        var r = (packed >> 16) & 0xFF
        var g = (packed >> 8) & 0xFF
        var b = packed & 0xFF
        return Tuple[Int32, Int32, Int32](r, g, b)

    fn _rgb_to_hsv(mut self):
    ...
    fn _handle_popup_click(mut self, mouse_x: Int32, mouse_y: Int32) -> Bool:
        """Handle clicks in popup area"""
        var popup_x = self.x
        var popup_y = self.y + self.height + 5
        var popup_width = max(Int32(300), self.width)
        var popup_height = Int32(250)
    ...
    fn _handle_wheel_click(mut self, mouse_x: Int32, mouse_y: Int32, popup_x: Int32, popup_y: Int32) -> Bool:
        ...
        var distance = Int32(sqrt(Float64(dx * dx + dy * dy)))

        if distance <= radius:
            # Calculate hue from angle
            var angle = atan2(Float64(dy), Float64(dx)) * 180.0 / 3.14159
            if angle < 0:
                angle += 360
            self.hue = Int32(angle)

            # Calculate saturation from distance
            self.saturation = min(Int32(100), (distance * 100) // radius)
    ...
    fn _handle_slider_click(mut self, mouse_x: Int32, mouse_y: Int32, popup_x: Int32, popup_y: Int32) -> Bool:
        ...
        if (mouse_y >= popup_y + 30 and mouse_y <= popup_y + 50):
            var pos = max(Int32(0), min(Int32(slider_width), mouse_x - slider_x))
    ...
    fn _handle_compact_click(mut self, mouse_x: Int32, mouse_y: Int32, popup_x: Int32, popup_y: Int32) -> Bool:
    ...
    fn _draw_rgb_sliders(self, lib: DLHandle, popup_x: Int32, popup_y: Int32):
        ...
        var red_label = "R: " + String(self.color_r)
        ...
        # Green slider
        var green_label = "G: " + String(self.color_g)
        ...
        # Blue slider
        var blue_label = "B: " + String(self.color_b)
