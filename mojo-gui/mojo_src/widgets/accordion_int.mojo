...
comptime ACCORDION_SINGLE = 0
comptime ACCORDION_MULTIPLE = 1
...
struct AccordionInt(WidgetInt, Copyable, Movable):
    ...
    fn render_section(self, ctx: RenderingContextInt, index: Int32):
        ...
        var section = self.sections[Int(index)]
    ...
    fn set_expansion_mode(mut self, mode: Int32):
    ...
    fn is_section_expanded(self, index: Int32) -> Bool:
        ...
        if index >= 0 and index < len(self.sections):
            return self.sections[Int(index)].is_expanded
        return False

    fn set_section_enabled(mut self, index: Int32, enabled: Bool):
        """Enable or disable a section."""
        if index >= 0 and index < len(self.sections):
            self.sections[Int(index)].enabled = enabled
