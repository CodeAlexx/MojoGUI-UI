...
struct ListItemInt(ImplicitlyCopyable, Movable):
    """Individual list item."""
    var text: String
    var data: Int32  # Optional data associated with item
    var enabled: Bool

    fn __init__(out self, text: String, data: Int32 = 0):
        self.text = text
        self.data = data
        self.enabled = True

struct ListBoxInt(WidgetInt, Copyable, Movable):
    ...
    fn add_item(mut self, text: String, data: Int32 = 0):
        """Add item to list."""
    ...
    fn ensure_visible(mut self, index: Int32):
        ...
        # Clamp scroll offset
        var max_scroll = len(self.items) - self.visible_items
    ...
    fn get_item_rect(self, item_index: Int32) -> RectInt:
        ...
        var relative_index = item_index - self.scroll_offset
        var item_y = self.bounds.y + self.border_width + relative_index * self.item_height
    ...
    fn item_index_from_point(self, point: PointInt) -> Int32:
        ...
        var content = self.get_content_rect()
        if not content.contains(point):
            return -1
        
        var relative_y = point.y - content.y
        var item_index = self.scroll_offset + relative_y // self.item_height
        
        if item_index >= 0 and item_index < Int32(len(self.items)):
            return item_index
        return -1
    ...
    fn handle_mouse_event(mut self, event: MouseEventInt) -> Bool:
        ...
        if inside:
            var item_index = self.item_index_from_point(point)
    ...
    fn handle_key_event(mut self, event: KeyEventInt) -> Bool:
        ...
        if event.pressed:
            if event.key_code == 265:  # Up arrow
                ...
                if self.selected_index < Int32(len(self.items)) - 1:
                    self.set_selected_index(self.selected_index + 1)
                return True
            elif event.key_code == 266:  # Page Up
                var new_index = max(Int32(0), self.selected_index - self.visible_items)
                self.set_selected_index(new_index)
                return True
            elif event.key_code == 267:  # Page Down
                var new_index = min(Int32(len(self.items)) - 1, self.selected_index + self.visible_items)
                self.set_selected_index(new_index)
                return True
                ...
                self.set_selected_index(Int32(len(self.items)) - 1)
                return True
    ...
    fn render(self, ctx: RenderingContextInt):
        ...
        var end_index = min(Int32(len(self.items)), self.scroll_offset + self.visible_items)
        
        # Draw visible items
        for i in range(Int(self.scroll_offset), Int(end_index)):
            if i >= len(self.items) or not self.items[i].enabled:
                continue
                
            var item_rect = self.get_item_rect(Int32(i))
            var is_selected = (Int32(i) == self.selected_index)
            var is_hover = (Int32(i) == self.hover_index) and not is_selected
            ...
            # Draw item text
            var text_color = self.selected_text_color if is_selected else self.text_color
            _ = ctx.set_color(text_color.r, text_color.g, text_color.b, text_color.a)
            
            var text_x = item_rect.x + self.padding
            var text_y = item_rect.y + (item_rect.height - self.font_size) // 2
            _ = ctx.draw_text(self.items[i].text, text_x, text_y, self.font_size)
        
        # Draw scrollbar if needed
        if Int32(len(self.items)) > self.visible_items:
    ...
    fn render_scrollbar(self, ctx: RenderingContextInt):
        ...
        var scrollbar_x = self.bounds.x + self.bounds.width - scrollbar_width - 1
        var content = self.get_content_rect()
        ...
        var total_items = len(self.items)
        var thumb_height = max(Int32(20), content.height * self.visible_items // Int32(total_items))
        var thumb_y = content.y + content.height * self.scroll_offset // Int32(total_items)
