...
# Menu item types
comptime MENU_ITEM_NORMAL = 0
comptime MENU_ITEM_SEPARATOR = 1
comptime MENU_ITEM_SUBMENU = 2
comptime MENU_ITEM_CHECKBOX = 3

# Menu states
comptime MENU_ITEM_ENABLED = 1
comptime MENU_ITEM_DISABLED = 2
comptime MENU_ITEM_CHECKED = 4

struct MenuItem(Copyable, Movable):
    """Context menu item."""
    var id: Int32
    var text: String
    var shortcut: String
    ...
    fn __init__(out self, id: Int32, text: String, item_type: Int32 = MENU_ITEM_NORMAL):
        self.id = id
        self.text = text
        self.shortcut = ""
...
struct ContextMenuInt(Copyable, Movable):
    ...
    # Explicit position/size fields used by this widget's logic.
    var x: Int32
    var y: Int32
    var width: Int32
    var height: Int32
    ...
    fn _point_in_bounds(self, point: PointInt) -> Bool:
        """Check if a point is within the menu's current bounds."""
        return (point.x >= self.x and point.x < self.x + self.width and
                point.y >= self.y and point.y < self.y + self.height)

    fn add_item(mut self, id: Int32, text: String, shortcut: String = "") -> Int32:
    ...
    fn add_separator(mut self) -> Int32:
    ...
    fn add_checkbox_item(mut self, id: Int32, text: String, checked: Bool = False) -> Int32:
    ...
    fn set_item_enabled(mut self, item_index: Int32, enabled: Bool):
    ...
    fn set_item_checked(mut self, item_index: Int32, checked: Bool):
    ...
    fn show_at(mut self, x: Int32, y: Int32, parent_id: Int32 = -1):
        """Show the context menu at the specified position."""
        self.show_at_bounded(x, y, 0, 0, 0, 0, parent_id)

    fn show_at_bounded(mut self, x: Int32, y: Int32,
    ...
    fn _recalculate_size(mut self):
        ...
        for i in range(self.item_count):
            if self.items[i].item_type != MENU_ITEM_SEPARATOR:
                ...
                var text_width = len(self.items[i].text) * Int(self.font_size * 6 // 10)
                var shortcut_width = len(self.items[i].shortcut) * Int(self.font_size * 6 // 10)
        ...
        self.calculated_width = self.item_padding * 2 + Int32(max_text_width)
        if max_shortcut_width > 0:
            self.calculated_width += Int32(max_shortcut_width) + 20  # Space between text and shortcut
    ...
    fn hit_test_item(self, x: Int32, y: Int32) -> Int32:
        ...
        var relative_y = y - self.y - self.item_padding
        var current_y = Int32(0)
        ...
        if item_index >= 0:
            ...
            if event.pressed:
                ...
                if (self.items[item_index].state & MENU_ITEM_ENABLED) != 0:
                    ...
                    if self.items[item_index].item_type == MENU_ITEM_CHECKBOX:
                        var checked = (self.items[item_index].state & MENU_ITEM_CHECKED) != 0
        ...
        if event.key_code == 256:  # Escape key
            ...
            if self.hovered_item >= 0:
                ...
                if self.items[self.hovered_item].item_type == MENU_ITEM_CHECKBOX:
                    var checked = (self.items[self.hovered_item].state & MENU_ITEM_CHECKED) != 0
    ...
    fn _navigate_previous(mut self):
        ...
        for i in range(self.item_count):
            var index = (start - i + self.item_count) % self.item_count
            if (self.items[index].item_type != MENU_ITEM_SEPARATOR and
                (self.items[index].state & MENU_ITEM_ENABLED) != 0):
                self.hovered_item = index
                return
    ...
    fn _draw_items(self, ctx: RenderingContextInt):
        ...
        for i in range(self.item_count):
            var item_y = current_y

            if self.items[i].item_type == MENU_ITEM_SEPARATOR:
                # Draw separator line
                var sep_y = item_y + self.separator_height // 2
                ...
                if self.items[i].item_type == MENU_ITEM_CHECKBOX:
                    var check_x = self.x + 6
                    var check_y = item_y + 3
                    var check_size = 12
                ...
                # Draw item text
                var text_x = self.x + (30 if self.items[i].item_type == MENU_ITEM_CHECKBOX else 12)
                var text_y = item_y + 4
                ...
                if len(self.items[i].shortcut) > 0:
                    var shortcut_x = self.x + self.width - 12 - Int32(len(self.items[i].shortcut)) * self.font_size * 6 // 10
...
struct ContextMenuManager(Copyable, Movable):
    ...
    fn show_menu(mut self, menu: ContextMenuInt, x: Int32, y: Int32):
        """Show a context menu."""
        self.active_menu = menu
        self.active_menu.show_at(x, y)
        self.menu_active = True

    fn hide_active_menu(mut self):
        """Hide the currently active menu."""
        if self.menu_active:
            self.active_menu.hide()
            self.menu_active = False

    fn handle_global_mouse(mut self, event: MouseEventInt) -> Bool:
        """Handle mouse events for active context menu."""
        if self.menu_active:
            return self.active_menu.handle_mouse_event(event)
        return False

    fn handle_global_key(mut self, event: KeyEventInt) -> Bool:
