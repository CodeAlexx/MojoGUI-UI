...
struct MenuItemInt(Copyable, Movable):
    ...
    fn __init__(out self, text: String, shortcut: String = "", data: Int32 = 0):
        self.text = text
...
struct MenuBarInt(WidgetInt, Copyable, Movable):
    ...
    fn add_menu(mut self, text: String, items: List[MenuItemInt]):
        ...
        var text_width = len(text) * (self.font_size * 6 // 10) + 2 * self.padding
    ...
    fn get_dropdown_rect(self, menu_index: Int32) -> RectInt:
        ...
        var menu_rect = self.get_menu_rect(menu_index)
        var item_count = len(self.menu_items[menu_index].submenu_items)
        var dropdown_height = item_count * self.dropdown_height
    ...
    fn dropdown_item_from_point(self, point: PointInt, menu_index: Int32) -> Int32:
        ...
        var dropdown_rect = self.get_dropdown_rect(menu_index)
        if not dropdown_rect.contains(point):
            return -1

        var relative_y = point.y - dropdown_rect.y
        var item_index = relative_y // self.dropdown_height
    ...
    fn handle_mouse_event(mut self, event: MouseEventInt) -> Bool:
        ...
        if event.pressed:
            ...
            var menu_index = self.menu_index_from_point(point)
            if menu_index >= 0:
            ...
            if self.dropdown_visible and self.active_menu >= 0:
                var dropdown_item = self.dropdown_item_from_point(point, self.active_menu)
                if dropdown_item >= 0:
    ...
    fn render(self, ctx: RenderingContextInt):
        ...
        # Draw menu items
        for i in range(len(self.menu_items)):
            var menu_rect = self.get_menu_rect(i)
            var is_active = (Int32(i) == self.active_menu)
            var is_hover = (Int32(i) == self.hover_item) and not is_active
            ...
            var text_x = menu_rect.x + self.padding
            var text_y = menu_rect.y + (menu_rect.height - self.font_size) // 2
    ...
    fn render_dropdown(self, ctx: RenderingContextInt, menu_index: Int32):
        ...
        # Draw dropdown items
        ref items = self.menu_items[menu_index].submenu_items
        for i in range(len(items)):
            var item_y = dropdown_rect.y + i * self.dropdown_height
            var is_hover = (Int32(i) == self.dropdown_hover)
            ...
            # Draw item text
            var text_color = self.disabled_color if not items[i].enabled else self.text_color
            ...
            if items[i].is_separator:
                ...
                # Draw text
                var text_x = dropdown_rect.x + self.padding
                var text_y = item_y + (self.dropdown_height - self.font_size) // 2
                _ = ctx.draw_text(items[i].text, text_x, text_y, self.font_size)

                # Draw shortcut if present
                if len(items[i].shortcut) > 0:
                    var shortcut_x = dropdown_rect.x + dropdown_rect.width - self.padding - len(items[i].shortcut) * 6
