...
# Status panel types
comptime STATUS_PANEL_TEXT = 0
comptime STATUS_PANEL_PROGRESS = 1
comptime STATUS_PANEL_ICON = 2
comptime STATUS_PANEL_CLOCK = 3

# Status panel styles
comptime STATUS_STYLE_FLAT = 0
comptime STATUS_STYLE_SUNKEN = 1
comptime STATUS_STYLE_RAISED = 2
...
struct StatusPanel(ImplicitlyCopyable, Movable):
    ...
    fn __init__(out self, id: Int32, text: String = "", width: Int32 = 100):
        self.id = id
        self.text = text
        self.width = width
        self.min_width = 50
...
struct StatusBarInt(WidgetInt, Copyable, Movable):
    ...
    fn __init__(out self, x: Int32, y: Int32, width: Int32, height: Int32 = 24):
    ...
    fn _point_in_bounds(self, point: PointInt) -> Bool:
        return self.bounds.contains(point)
    ...
    fn add_panel(mut self, text: String, width: Int32 = 100, auto_size: Bool = True) -> Int32:
    ...
    fn add_progress_panel(mut self, width: Int32 = 150, max_value: Int32 = 100) -> Int32:
    ...
    fn add_clock_panel(mut self, width: Int32 = 80) -> Int32:
    ...
    fn set_panel_text(mut self, panel_index: Int32, text: String):
    ...
    fn set_panel_progress(mut self, panel_index: Int32, value: Int32, max_value: Int32 = -1):
    ...
    fn set_panel_width(mut self, panel_index: Int32, width: Int32):
    ...
    fn set_panel_style(mut self, panel_index: Int32, style: Int32):
        """Set visual style for a specific panel."""
        if panel_index >= 0 and panel_index < self.panel_count:
            self.panels[panel_index].style = style

    fn set_panel_visible(mut self, panel_index: Int32, visible: Bool):
        """Show or hide a specific panel."""
        if panel_index >= 0 and panel_index < self.panel_count:
            self.panels[panel_index].visible = visible
            self._recalculate_layout()

    fn remove_panel(mut self, panel_index: Int32):
    ...
    fn _recalculate_layout(mut self):
        """Recalculate panel positions and sizes."""
        self.available_width = self.bounds.width - (self.grip_size if self.show_size_grip else 0)

        # First pass: calculate fixed widths and count auto-size panels
        var used_width: Int32 = 0
        var auto_size_count: Int32 = 0
        ...
        for i in range(self.panel_count):
            ...
            if self.panels[i].auto_size:
                ...
                var min_text_width = Int32(len(self.panels[i].text)) * (self.font_size * 6 // 10) + 16
        ...
        var remaining_width = self.available_width - used_width
        var auto_size_extra = remaining_width // max(Int32(1), auto_size_count) if auto_size_count > 0 else 0

        # Third pass: set panel bounds
        var current_x = self.bounds.x + self.border_width
        ...
        for i in range(self.panel_count):
            ...
            var panel_height = self.height - (self.border_width * 2)
            self.panels[i].bounds = RectInt(current_x, self.bounds.y + self.border_width,
                                          panel_width, panel_height)

            current_x += panel_width + self.panel_spacing

        self.content_width = current_x - self.bounds.x
    ...
    fn hit_test_grip(self, x: Int32, y: Int32) -> Bool:
        ...
        var grip_x = self.bounds.x + self.bounds.width - self.grip_size
        return (x >= grip_x and x <= self.bounds.x + self.bounds.width and
                y >= self.bounds.y and y <= self.bounds.y + self.height)
    ...
    fn handle_mouse_event(mut self, event: MouseEventInt) -> Bool:
        ...
        var panel_index = self.hit_test_panel(event.x, event.y)
    ...
    fn draw(self, ctx: RenderingContextInt):
        ...
        _ = ctx.draw_filled_rectangle(self.bounds.x, self.bounds.y, self.bounds.width, self.height)

        # Draw top border
        _ = ctx.set_color(self.border_color.r, self.border_color.g, self.border_color.b, self.border_color.a)
        _ = ctx.draw_filled_rectangle(self.bounds.x, self.bounds.y, self.bounds.width, 1)
    ...
    fn _draw_panel_text(self, ctx: RenderingContextInt, panel_index: Int32):
        ...
        var text_y = bounds.y + (bounds.height - panel.font_size) // 2

        if panel.alignment == ALIGN_CENTER:
            var text_width = Int32(len(panel.text)) * (panel.font_size * 6 // 10)
    ...
    fn _draw_panel_progress(self, ctx: RenderingContextInt, panel_index: Int32):
        ...
        if panel.progress_value > 0 and panel.progress_max > 0:
            var progress_width = (bounds.width - margin * 2) * panel.progress_value // panel.progress_max
        ...
        # Progress text
        var percent = panel.progress_value * 100 // panel.progress_max if panel.progress_max > 0 else 0
        var progress_text = String(percent) + "%"
        var text_width = Int32(len(progress_text)) * (panel.font_size * 6 // 10)
        var text_x = bounds.x + (bounds.width - text_width) // 2
        var text_y = bounds.y + (bounds.height - panel.font_size) // 2
    ...
    fn _draw_size_grip(self, ctx: RenderingContextInt):
        """Draw the size grip."""
        var grip_x = self.bounds.x + self.bounds.width - self.grip_size
        var grip_y = self.bounds.y

        # Draw grip pattern (diagonal lines)
        _ = ctx.set_color(self.border_color.r, self.border_color.g, self.border_color.b, self.border_color.a)

        for i in range(3):
            var offset = Int32(i) * 4 + 4
            var line_x = grip_x + offset
            var line_y = grip_y + self.height - offset

            # Draw diagonal line
            for j in range(Int(min(offset, self.grip_size - offset))):
                _ = ctx.draw_filled_rectangle(line_x + Int32(j), line_y + Int32(j), 1, 1)
