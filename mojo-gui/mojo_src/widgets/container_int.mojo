...
comptime LAYOUT_VERTICAL = 1
comptime LAYOUT_HORIZONTAL = 2
comptime LAYOUT_GRID = 3
...
struct ChildWidget(Copyable, Movable):
...
struct ContainerInt(WidgetInt, Copyable, Movable):
    ...
    fn add_child(mut self, widget_id: Int32, w: Int32, h: Int32, flex_grow: Int32 = 0) -> Int32:
    ...
    fn remove_child(mut self, widget_id: Int32) -> Bool:
    ...
    fn set_child_margin(mut self, widget_id: Int32, left: Int32, top: Int32, right: Int32, bottom: Int32):
    ...
    fn set_padding(mut self, left: Int32, top: Int32, right: Int32, bottom: Int32):
    ...
    fn set_spacing(mut self, spacing: Int32):
        """Set spacing between child widgets."""
        self.spacing = spacing
        self.calculate_layout()

    fn set_grid_size(mut self, columns: Int32, rows: Int32):
    ...
    fn calculate_layout(mut self):
        ...
        var content_x = self.bounds.x + self.padding_left
        var content_y = self.bounds.y + self.padding_top
        var content_w = self.bounds.width - self.padding_left - self.padding_right
        var content_h = self.bounds.height - self.padding_top - self.padding_bottom
    ...
    fn _layout_vertical(mut self, start_x: Int32, start_y: Int32, width: Int32, height: Int32):
    ...
    fn _layout_horizontal(mut self, start_x: Int32, start_y: Int32, width: Int32, height: Int32):
    ...
    fn _layout_grid(mut self, start_x: Int32, start_y: Int32, width: Int32, height: Int32):
        """Arrange children in a grid."""
        if self.grid_columns <= 0 or self.grid_rows <= 0:
            return

        var cell_width = (width - (self.grid_columns - 1) * self.spacing) // self.grid_columns
        var cell_height = (height - (self.grid_rows - 1) * self.spacing) // self.grid_rows
        ...
        for i in range(self.child_count):
            ...
            var row = visible_count // self.grid_columns
            var col = visible_count % self.grid_columns
    ...
    fn handle_mouse_event(mut self, event: MouseEventInt) -> Bool:
        ...
        for i in range(self.child_count - 1, -1, -1):
            ...
            var child_bounds = RectInt(self.children[i].x, self.children[i].y,
                                      self.children[i].width, self.children[i].height)
    ...
    fn handle_key_event(mut self, event: KeyEventInt) -> Bool:
        """Handle key events (containers are typically non-interactive)."""
        return False

    fn draw(self, ctx: RenderingContextInt):
        """Draw the container and manage child rendering."""
        # Draw container background + border (inlined from BaseWidgetInt)
    ...
    fn render(self, ctx: RenderingContextInt):
        """Render the container (WidgetInt conformance)."""
        if not self.visible:
            return
        self.draw(ctx)

    fn update(mut self):
        """Update container state (called each frame)."""
        pass
    ...
    fn set_scroll_offset(mut self, x: Int32, y: Int32):
