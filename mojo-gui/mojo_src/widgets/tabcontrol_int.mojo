...
struct TabPageInt(Copyable, Movable):
    """Individual tab page."""
    var title: String
    ...
    fn __init__(out self, title: String, data: Int32 = 0):
        self.title = title
...
struct TabControlInt(WidgetInt, Copyable, Movable):
    """Tabbed interface widget using integer coordinates."""
    ...
    fn add_tab(mut self, title: String, data: Int32 = 0):
        """Add a new tab."""
        self.tabs.append(TabPageInt(title, data))

        # Calculate tab width
        var text_width = len(title) * (self.font_size * 6 // 10) + 2 * self.tab_padding
    ...
    fn remove_tab(mut self, index: Int32):
        """Remove tab at index."""
    ...
    fn set_active_tab(mut self, index: Int32):
        """Set active tab."""
    ...
    fn handle_mouse_event(mut self, event: MouseEventInt) -> Bool:
        ...
        if event.pressed and inside:
            var tab_index = self.tab_index_from_point(point)
    ...
    fn render_tab(self, ctx: RenderingContextInt, tab_index: Int32):
        ...
        var tab_rect = self.get_tab_rect(tab_index)
        var is_active = (tab_index == self.active_tab)
        var is_hover = (tab_index == self.hover_tab) and not is_active
        var is_enabled = self.tabs[tab_index].enabled
        ...
        # Draw tab text
        var text_color = self.tab_text_color if is_enabled else self.tab_disabled_color
        _ = ctx.set_color(text_color.r, text_color.g, text_color.b, text_color.a)

        var text_x = tab_rect.x + self.tab_padding
        var text_y = tab_rect.y + (tab_rect.height - self.font_size) // 2
        ...
        if self.close_buttons and is_enabled:
            var close_x = tab_rect.x + tab_rect.width - 20
            var close_y = tab_rect.y + 8
    ...
    fn render_tab_content(self, ctx: RenderingContextInt, tab_index: Int32):
        ...
        var content_text = "Content for: " + self.tabs[tab_index].title
        var content_x = content_rect.x + 10
        var content_y = content_rect.y + 20
        _ = ctx.draw_text(content_text, content_x, content_y, self.font_size + 2)

        var info_text = "This is the content area for tab " + String(tab_index)
        _ = ctx.draw_text(info_text, content_x, content_y + 30, self.font_size)

    fn update(mut self):
        """Update tab control state."""
        # Ensure we have a valid active tab
        if self.active_tab >= len(self.tabs) or (self.active_tab >= 0 and (not self.tabs[self.active_tab].enabled or not self.tabs[self.active_tab].visible)):
